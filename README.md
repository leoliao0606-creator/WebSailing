...
6 自由度（前进/横漂/艏摇/横摇/升沉/纵摇）体轴动力学，120Hz 定步长积分：

- **帆**：软翼对称升阻力曲线（换舷拱度翻面），失速后过渡到平板阻力特性；
  帆杠受缭绳约束自然摆向下风，正顺风带换舷滞回（可以背风行驶）。出生/重置时
  帆杠瞬间落到当前风况下的位置——缭绳放尽而帆杠还收在中线是不自洽的状态，
  让它慢慢摆出去的话，大风横风起步的头半秒帆面几乎正对风，能把船直接掀翻。
- **水下附体**：稳向板/舵叶为有限展弦比对称翼，失速角 ~16-24°——
  低速大侧滑时板失速，出弯横漂、舵满舵失效都是涌现行为。
  死区顶死时船会倒漂，来流从艉打来使**舵效自然反打**（玩家脱困要自己反舵，
  HUD 有倒航提示；AI 与辅助转向已内置这一步，否则大风调向会永远卡在顶风点）。
- **船体阻力**：黏性 + Froude≈0.44 兴波阻力峰（排水航行的"墙"）+ 滑行减阻，
  强风横风可破墙滑行（>10 节）。倒着走另算：方形船尾正面破水是钝体而非流线型，
  形状阻力约为正向的十几倍，顶风停住的船因此只以约 1 节缓缓倒退。
...
- **风影**：上风船的帆向下风投出 ~7 倍桅高的乱流带（风速最多 -42%），
  盖风/抢清风的竞速战术由此涌现。
- **升沉/纵摇**：沿船长 5 站的水线面浮力驱动，船有自己的惯性和固有周期
  （升沉 ~0.53 s、纵摇 ~0.60 s），不是贴在浪面上的贴纸。于是短于船长的碎浪被
  各站平均掉、船跨过去而不跟着抖；冲上浪顶后浸没归零 → 浮力消失 → 腾空 →
  落回砸水（腾空时舵效大幅下降）。
- **波浪**：12 分量 Gerstner —— 10 个随风速成长的风浪（方向围绕风向散布，波长越短
  散得越开）+ 2 个方向与本地风无关的涌浪（小风时海面也不会变成镜子）。每个分量带
  固定相位常数，否则所有波会在原点同相叠加出规则的搓板纹。物理采样与 GPU 顶点位移
  共用同一组参数（含水平位移反解）。
  波浪真实推船 —— 水质点轨道流速按各分量自己的波数做深度衰减后进入所有水动力项
  （碎浪在吃水处几乎没有流速，长浪几乎不衰减），浪面坡度产生冲浪推力，
  冲浪时船体卸载（兴波阻力下降）使追浪跃上滑行成为核心顺风技巧。
- **顶浪掉速**：船在浪里起落做功、把能量辐射成波，表现为掉速。顶浪时船与浪相向
  而行、遭遇频率高、艏部起落剧烈，这项显著；顺浪时船跟着浪走，艏部相对水面几乎
  不动，它自动趋近于零。两种航向的差别是物理给出的，不是分别写的系数。
...
默认使用**高**画质。四档共同保留清透天空、时段光照和航行所需的阵风暗纹，
按实际渲染开销分配细节；画质不会改变风浪物理或联机状态。

| 预设 | 分辨率倍率 | 阴影贴图 | 水面三角形 | 云层细节 | 纹理过滤 | 浪花 / 尾流 | 动态分辨率 |
| --- | --- | --- | ---: | --- | --- | --- | --- |
| 低 | 70% | 关闭 | 15,264 | 默认关闭 | 1× | 关闭 | 开 |
| 中 | 85% | 1024² | 40,800 | 基础 | 4× | 开 | 开 |
| 高（默认） | 100% | 2048² | 85,792 | 精细 | 8× | 开 | 开 |
| 超高 | 130% | 4096² | 245,376 | 最精细 | 16× | 开 | 关 |

| 预设 | 船壳纵向 × 横向分段 | 帆面分段 | 每船粒子容量 | 尾流采样点 |
| --- | --- | --- | ---: | ---: |
| 低 | 32 × 20 | 16 × 12 | 256（手动开启特效时） | 48 |
| 中 | 56 × 32 | 24 × 16 | 768 | 80 |
| 高 | 88 × 48 | 36 × 24 | 2,048 | 128 |
| 超高 | 144 × 64 | 56 × 36 | 6,144 | 208 |

分辨率倍率作用于设备像素比（设备像素比最高取 2）；纹理过滤受显卡支持上限约束。
超高档比高档多约 69% 像素、2.9 倍水面三角形、4 倍阴影像素、3 倍粒子容量，
适合有独立显卡的设备；它是唯一关掉动态分辨率的一档，掉帧时不会自动降画面。
这是一组渲染预算，不代表某款显卡的帧率保证。

- **天空与云层**：天空保持线性颜色直至最终输出，避免重复提亮造成发白；
  云层随风移动，具备迎光边缘与底部暗部，并进入环境光照与反射贴图。
- **海面**：低至超高使用 80×96、128×160、192×224、256×320 的径向网格，
  近处密、远处疏，覆盖半径 16 km。中档开始增加风向细波纹，
  高 / 超高增加经过粗糙度过滤的天空云层倒影、细碎白沫；过滤避免倒影边缘闪烁。
  帆船的阴影可投射到起伏的水面上。
- **材质**：1024² 帆面纹理、织物凹凸与柔和反光，甲板防滑凹凸，岛屿地表纹理和坡面岩石颜色。
  纹理清晰度控制斜视角下的过滤质量。
- **船体与船员**：平滑船体型线、圆角舵板和稳向板；中档起增加滑轮、导缆器、螺栓、
  绳圈和压舷带。水手有救生衣分片、肩带、反光条、面部、帽檐、手套与船鞋，
  保留压舷和操帆动作。静态甲板五金按材质合并绘制，切换模型档位会释放旧几何与材质。
- **浪花与尾流**：普通航速即产生船艏水滴和碎沫，强度随前进速度、滑行和砸水增加。
  GPU 计算水滴抛物线与泡沫逐顶点的波面位移；中央尾流和两侧扩散波随时间消散。
  粒子容量是上限，实际数量取决于航行状态；停船停止持续喷浪，重置或传送会清除旧尾迹。
- **独立设置**：分辨率可选 50%–150%，阴影 / 水面 / 云层 / 纹理 / 模型 / 粒子密度可分别调整，
  改动细项自动切换为“自定义”。云层和浪花 / 尾流可关闭。
- **时段天气**：晴空斜阳 / 正午 / 暮色 / 阴天，独立于画质，纯本地视觉、联机不同步。
- **动态分辨率**：掉帧时最多降至所选倍率的 75%，性能恢复后升回；切档时从该档完整分辨率重新评估。
  超高默认关闭以保持清晰度，也可单独开启。
- **浏览器性能信息**：启用“性能信息”可查看帧率、渲染分辨率、三角形、活动粒子数及实际
  WebGL 渲染设备名称；支持计时扩展时显示 GPU 渲染毫秒，未提供时显示“—”。
  检测到 SwiftShader / llvmpipe 等软件渲染时会提示。创建渲染器时请求 `high-performance`，
  具体 GPU 仍由浏览器和系统决定。单块显卡的系统占用率不能代替页面实际渲染设备信息。
  设备名称与计时分别使用 [WEBGL_debug_renderer_info](https://developer.mozilla.org/en-US/docs/Web/API/WEBGL_debug_renderer_info)
  和 [EXT_disjoint_timer_query_webgl2](https://registry.khronos.org/webgl/extensions/EXT_disjoint_timer_query_webgl2/)。
- **旧设置迁移**：旧的低 / 中 / 高 / 超高预设会升级到新预算；自定义细项、天气、音量保持原值。

渲染验证：`npm run test:rendering`。使用已安装的 Playwright Chromium 逐档检查着色器、
阴影、几何细节、纹理过滤、连续低速 / 高速浪花、暂停与重置、性能信息、保存 / 重载、语言和窄屏设置页，
同时验证分档不改变波场参数。截图和报告默认保存到 `/tmp/windchaser-rendering`，
可用 `RENDER_ARTIFACT_DIR` 指定其他目录；此测试不测定硬件性能。
...
- 滑行抬艏是渲染层的姿态偏置而非力矩（物理上加常量抬艏力矩会让前半船离水、
  纵摇恢复力矩消失，一路顶到姿态上限卡死）；波浪方向谱用 12 个离散分量近似，
  从高空俯视仍能看出交叉波的规则纹理。
- 28 节以上直接抢风调向会失败：迎风段的帆已被压到几乎无动力，进入速度不足以
  冲过顶风点。这是真实的——先落下风把速度提到 5 节以上再调向就能转过来。
- 无护航/障碍物权利（RRS 19-20）、起航抢线权利（RRS 15-16）、抗议流程；
  处罚为回转或减速，非真实记分。
